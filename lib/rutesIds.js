...
async function rutesIds() {
    ...
    return buenas;
/*     resChildrens.results.forEach(o=> 
      o.type === 'child_page' ? 
          listIDs.push(o.id) : false
      ); */
...
/*   let completListIDs = [];
  ...
  resChildrens.results.forEach(o=> 
    ...
    for(let x=0 ; x<listIDs.length ; x++) {
        const blockId = listIDs[x];
        //const resContainer = await notion.blocks.retrieve({block_id: blockId});
        console.log('blockId:' + blockId);
        resChildrens = await notion.blocks.children.list({block_id: blockId});
        console.log('Num Ciclo:' + x);
        ...
        completListIDs = completListIDs.concat(listIDs);
        console.log(listIDs);
        listIDs = [];
    }
  ...
  let objParamsIds = [];
  completListIDs.forEach(e => objParamsIds.push({params:{id: e}}));
  return objParamsIds; */
/*   return [
